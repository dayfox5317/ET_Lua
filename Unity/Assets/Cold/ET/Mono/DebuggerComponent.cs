...
namespace ETCold
{
    ...
    public class DebuggerComponent: MonoBehaviour
    {
        ...
        private void ExpansionGUIWindow(int windowId)
        {
            ...
            if (GUILayout.Button("PageUp", GUILayout.Height(30)))
            {
                _scrollLogView.y -=pageHeight-20;
            }
            if (GUILayout.Button("PageDown", GUILayout.Height(30)))
            {
                _scrollLogView.y +=pageHeight-20;
            }
            ...
            if (_debugType == DebugType.Console)
            {
                ...
                _scrollLogView = GUILayout.BeginScrollView(_scrollLogView,"Box",GUILayout.Height(pageHeight));
...
#else
            GUILayout.Label("总内存：" + UnityEngine.Profiling.Profiler.GetTotalReservedMemoryLong() / 1000000 + "MB");
            GUILayout.Label("已占用内存：" + UnityEngine.Profiling.Profiler.GetTotalAllocatedMemoryLong() / 1000000 + "MB");
            GUILayout.Label("空闲中内存：" + UnityEngine.Profiling.Profiler.GetTotalUnusedReservedMemoryLong() / 1000000 + "MB");
            GUILayout.Label("总Mono堆内存：" + UnityEngine.Profiling.Profiler.GetMonoHeapSizeLong() / 1000000 + "MB");
            GUILayout.Label("已占用Mono堆内存：" + UnityEngine.Profiling.Profiler.GetMonoUsedSizeLong() / 1000000 + "MB");
#endif
