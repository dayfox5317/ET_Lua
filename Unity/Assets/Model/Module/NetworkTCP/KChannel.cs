 ...
 namespace ET
{
	...
	public class KChannel : AChannel
	{
		...
		public KChannel(long id, uint localConn, Socket socket, IPEndPoint remoteEndPoint, KService kService)
		{
			...
			this.socket = socket;
			this.kcp = new KcpWraper(RemoteConn, localConn);// Kcp.KcpCreate(this.RemoteConn, (IntPtr) this.LocalConn);
		...
		public KChannel(long id, uint localConn, uint remoteConn, Socket socket, IPEndPoint remoteEndPoint, KService kService)
		{
			...
			this.RemoteAddress = remoteEndPoint;
			this.socket = socket;
			this.kcp = new KcpWraper(remoteConn, localConn);// Kcp.KcpCreate(this.RemoteConn, (IntPtr) localConn);
...
#region 网络线程
		...
		public override void Dispose()
		{
			...
			if (this.kcp.IsEnable())
			{
				kcp.Release();
				// Kcp.KcpRelease(this.kcp);
				this.kcp = null;// IntPtr.Zero;
			}
		...
		public void HandleConnnect()
		{
			...
			this.kcp = new KcpWraper(RemoteConn, LocalConn);// Kcp.KcpCreate(this.RemoteConn, new IntPtr(this.LocalConn));
			this.InitKcp();
		...
		public void Update()
		{
			...
			if (this.kcp.IsEnable()/* != IntPtr.Zero*/)
			{
				uint nextUpdateTime = kcp.Check(timeNow);// Kcp.KcpCheck(this.kcp, timeNow);
		...
		public void HandleRecv(byte[] data, int offset, int length)
		{
			...
			while (true)
			{
				...
				int n = kcp.Peeksize();// Kcp.KcpPeeksize(this.kcp);
				if (n < 0)
				{
					break;
				}
				...
				int count =  kcp.Recv(buffer,n);// Kcp.KcpRecv(this.kcp, buffer, n);
				if (n != count)
				{
					break;
				}
		...
		public void Output_Safe(byte[] bytes)
		{
			if (this.IsDisposed)
			{
				return;
			...
			//	Marshal.Copy(bytes, buffer, 5, count);
				this.socket.SendTo(buffer, 0, bytes.Length + 5, SocketFlags.None, this.RemoteAddress);
			}
			catch (Exception e)
			{
				Log.Error(e);
        ...
        private void KcpSend(KcpWaitPacket kcpWaitPacket)
		{
			...
			kcp.Send(memoryStream.GetBuffer(), (int)memoryStream.Position, count);
		...
		public void Send(long actorId, MemoryStream stream)
		{
			if (this.kcp.IsEnable())
			{
				...
				int n = kcp.Waitsnd();// Kcp.KcpWaitsnd(this.kcp);
