...
{
    class MapDecorator1 : ProtoDecoratorBase //where TDictionary : class, IDictionary<TKey, TValue>
    {
        ...
        public Type TDictionary, TKey, TValue;
        internal MapDecorator1(TypeModel model, Type concreteType, IProtoSerializer keyTail, IProtoSerializer valueTail,
                 ...
                 (IProtoSerializer)new TagDecorator(2, valueWireType, false, valueTail))
        {
            ...
            this.keyTail = new DefaultValueDecorator(model,GetDefaultKey(), new TagDecorator(1, keyWireType, false, keyTail));
        ...
        private  Object GetDefaultValue() {
           ...
           return (TValue == typeof(string)) ? (object)"" : "";

        } 
        public override Type ExpectedType => TDictionary;// typeof(TDictionary);
        ...
        private object  GetDefaultKey()
        {
          ...
          return  (TKey == typeof(string)) ? (object)"" : "";
        ...
        public override object Read(object untyped, ProtoReader source)
        {
            ...
            var typed = AppendToCollection ? (System.Collections.IDictionary)(untyped) : null;
            if (typed == null) typed = (System.Collections.IDictionary)PType.CreateInstance(concreteType);
            ...
            do
            {
                ...
                while ((field = source.ReadFieldHeader()) > 0)
                {
                    ...
                    {
                        case 1:
                            key = keyTail.Read(null, source);
                            break;
                        case 2:
                            value = Tail.Read(Tail.RequiresOldValue ? (object)value : null, source);
                            break;
                        default:
                            source.SkipField();
                            break;
        ...
        public override void Write(object untyped, ProtoWriter dest)
        {
            ...
            foreach (var pair in (System.Collections.IEnumerable)untyped)
            {
                var k=pair.GetType().GetProperty("Key")?.GetValue(pair);
                var v = pair.GetType().GetProperty("Value")?.GetValue(pair);

                ProtoWriter.WriteFieldHeader(fieldNumber, wireType, dest);
                var token = ProtoWriter.StartSubItem(null, dest);
                if (k != null) keyTail.Write(k, dest);
                if (v != null) Tail.Write(v, dest);
