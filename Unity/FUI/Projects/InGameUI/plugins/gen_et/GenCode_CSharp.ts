...
function genCode(handler: FairyEditor.PublishHandler) {
    ...
    let old_dirs= System.IO.Directory.GetDirectories(exportCodePath);
    for (let index = 0; index < old_dirs.Length; index++) {
        const element = old_dirs[index];
        let files = System.IO.Directory.GetFiles(element);
        
        for (let index0 = 0; index0 < files.Length; index0++) {
            const element0 = files[index0];
            System.IO.File.Delete(element0);
        }
        System.IO.Directory.Delete(element);
        
    }

    let template0 = System.IO.File.ReadAllText(handler.project.assetsPath + "/../plugins/gen_et/Template.txt");
    ...
    for (let i: number = 0; i < classCnt; i++) {
        ...
        if(!System.IO.Directory.Exists(exportCodePath+'/'+classInfo.className)){
            System.IO.Directory.CreateDirectory(exportCodePath+'/'+classInfo.className);
        }

        System.IO.File.WriteAllText(exportCodePath+'/'+classInfo.className+'/'+ classInfo.className +'.cs',template);
    }
